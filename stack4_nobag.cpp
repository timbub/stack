...
void dump(struct Stack_t* stk, FILE* in)
{
    ...
    {
        stk->data[i] == TOXIC ? fprintf(in, "data[%d] = %d (TOXIC) || pointer data[%d] = %p\n", i, stk->data[i], i , &stk->data[i]) : fprintf(in, "data[%d] = %d || pointer data[%d] = %p\n", i, stk->data[i], i, &stk->data[i]);
    }
}
...
void verification(struct Stack_t* stk)
{
    ...
    if (stk->data[0] != L_CANARY) {
        error_number = 1;
        fprintf(stk->input_log, "Error: stk->l_data_canary != L_CANARY\n");
    }
    if(stk->data[stk->capacity + 1] != R_CANARY) {
        error_number = 1;
